...
eval fs = everywhereM b i
  ...
  f lbl (SD.TextBox tb) = SD.TextBox <<< M.fromMaybe tb <$> nbeTextBox tb
    ...
    nbeTextBox
      ∷ SD.TextBox (Compose M.Maybe SD.Expr)
      → m (M.Maybe (SD.TextBox (Compose M.Maybe SD.Expr)))
    nbeTextBox = evalTextBox >>> map (map quoteTextBox)

    evalTextBox
      ∷ SD.TextBox (Compose M.Maybe SD.Expr) → m (M.Maybe (SD.TextBox Identity))
    evalTextBox tb' = T.sequence $ fs.textBox lbl <$> asCode tb' <|> pure <$>
      asLit tb'
      where
      asLit = SD.traverseTextBox
        (unwrap >>> (_ >>= SD.getLiteral) >>> map Identity)
      asCode = SD.traverseTextBox
        (unwrap >>> (_ >>= SD.getUnevaluated) >>> map Const)

    quoteTextBox ∷ SD.TextBox Identity → SD.TextBox (Compose M.Maybe SD.Expr)
    quoteTextBox = SD.transTextBox
      (unwrap >>> SD.Literal >>> M.Just >>> Compose)
  ...
  f lbl (SD.DropDown msel opts) = do
    ...
    opts' ← evalExpr lbl fs.list opts
    pure $ SD.DropDown msel' $ M.maybe opts'
      (flip mergeSelection opts' <<< map L.singleton)
      msel'
